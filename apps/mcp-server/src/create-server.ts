...
export function createCodeOracleMcpServer(opts: CreateCodeOracleMcpServerOpts): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Find architectural decisions (WHY) for a topic. Returns summaries, alternatives, extract confidence, fused retrievalScore (RRF), and a mandatory source URL (PR, commit, or decision-shaped doc blob).",
      inputSchema: {
